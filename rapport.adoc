...
The user should be able to authenticate with his account on the website. The authentication should be handled by keycloak so the user can use the same account they created on the website.
...
The backend for frontend pattern is a way to centralize the communication with the backend services in a single service. This service is the only one the frontend needs to communicate with, it can then forward the requests to the different services.
